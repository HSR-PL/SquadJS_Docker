...
export default class DiscordServerStatus extends DiscordBaseMessageUpdater {
  ...
  static get optionsSpecification() {
    return {
      ...
      showNextLayerLayout: {
        required: false,
        description: "Whether to show next map layer.",
        default: true
      }
    };
  }
  ...
  async generateMessage() {
    ...
    const embed = new EmbedBuilder()
      ...
      if (this.options.showNextLayerLayout && this.server.nextLayer) {
        embed.setThumbnail(`https://squad-data.nyc3.cdn.digitaloceanspaces.com/main/${this.server.nextLayer.layerid}.jpg?size=2048`)
      }

      if (this.options.showJoinServerLink) {
        embed.addFields(
          'Join', `steam://connect/${this.server.options.host}:${this.server.options.queryPort}/`
        );
      }

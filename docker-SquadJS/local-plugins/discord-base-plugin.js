...
export default class DiscordBasePlugin extends BasePlugin {
  ...
  async sendDiscordMessage(message, labels) {
    try {
      ...
      console.log(`Attempting to send message with labels: ${JSON.stringify(labels)}`);
      console.log(`Message: ${JSON.stringify(message)}`);
      console.log(`cLabels: ${JSON.stringify(cLabels)}`);
      console.log(`Channels map: ${JSON.stringify([...this.channels])}`);

      for (const label of cLabels) {
          console.log(`Processing label: ${label}`);
          const c = this.channels.get(label);
          console.log(`Channel object for label: ${JSON.stringify(c)}`);
          if (this.isEmpty(c)) {
              console.log(`No channel object for label: ${label}`);
              continue;
          }
          if (this.isEmpty(c.channel)) {
              console.log(`Channel not initialized for label: ${label}`);
              this.dbg(
                  'DiscordJS',
                  1,
          ...
          console.log(`Attempting to send message to channel: ${c.channel.id}`);
          toSend.push(c.channel.send(message));
          console.log(`Message sent to channel: ${c.channel.id}`);
      }
